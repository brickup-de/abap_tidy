...
which [may hit you unprepared when not explicitly designing for it](/classes/scope/final-if-not-designed-for-inheritance/).
...
[resort to test seams instead](/testing/injection/use-test-seams-as-temporary-workaround/).
...
Common best practices include [resorting to other test tools](/testing/injection/exploit-the-test-tools/)
and extracting the problem method to a separate class with its own interface.

> A more specific variant of
> [Don't change the production code to make the code testable](/testing/injection/dont-add-features-to-production-code-that-are-only-intended-for-use-during-automated-testing/).
